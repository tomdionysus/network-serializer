class BinarySerializer {
  constructor (options = {}) {
    ...
    this.buffer = options.buffer || Buffer.alloc(this.chunkSize)
  ...
  writeDouble (value, bigEndian) {
    if(typeof(bigEndian) === 'undefined') bigEndian = this.bigEndian
  ...
  writeBigUInt64 (value, bigEndian) {
    if(typeof(bigEndian) === 'undefined') bigEndian = this.bigEndian
    this._ensureHeadroom(8)
    if (bigEndian) {
  ...
  getAbsoluteOffset () {
    var o = this.offset; var p = this.parent
    while (p) {
      o += p.offset
      p = p.parent
    }
